...
$(document).ready(function () {
    ...
    //gst = $("#gstval").val();
    if ($('.tbPurchaseOrder').hasClass('tbPurchaseOrder')) {
        let connection = new signalR.HubConnectionBuilder().withUrl("/signalServer").build();

        connection.start();

        connection.on("refreshPurchaseOrder", function () {
            loadData();
        });
        loadData();
    }
...
function LoadBillingAddress(vendor) {
    ...
    if ($(vendor).val() != "00000000-0000-0000-0000-000000000000") {
        $.ajax({
            type: "GET",
            url: "/PurchaseOrder/bindvendorAddressById",
...
$('#submit').click(function () {
    ...
    if ($('#PurchaseOrderNo').val().trim() == '') {
        $('#PurchaseOrderNo').siblings('span.error').css('visibility', 'visible');
        isValid = false;
    }
    else {
        $('#PurchaseOrderNo').siblings('span.error').css('visibility', 'hidden');
    }
    ...
    $('#orderdetailsItems tbody tr').each(function (index, ele) {
        if (
            ...
            var orderItem = {
                ...
                PurchaseOrderDetailsId: parseInt($(this).find("input[type='hidden']").val() == null ? 0 : $(this).find("input[type='hidden']").val()),
                ProductAndServiceId: $('select.productline', this).val(),
                Qty: parseInt($('.quantityline', this).val()),
                UnitPrice: parseFloat($('.unitpriceline', this).val()),
                DiscountPercent: parseFloat($('.discountline', this).val()),
    ...
    $.ajax({
        ...
        success: (result) => {
            $.each(result, (k, v) => {
                tr = tr + `<tr>
                        <td>
                            <a href="/Account/Edit/${v.PurchaseOrderId}">Edit</a>
                            <a onclick="return confirm ('Are you sure want to delete this PurchaseOrder?');" href="/Account/Delete?PurchaseOrderId=${v.PurchaseOrderId}">Delete</a>
                        </td>
                        <td>${v.PurchaseOrderNo}</td>
